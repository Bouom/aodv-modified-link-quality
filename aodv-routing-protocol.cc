...
/*std::vector<std::vector<double>> ReadLinkQualityFile(const std::string& filename) {
  std::ifstream file(filename);
  std::vector<std::vector<double>> linkQuality;
  std::string line;
  ...
  while (std::getline(file, line)) {
    ...
    linkQuality.push_back(row);
  }

  return linkQuality;
} */

// Implémentation de la méthode pour lire le fichier de qualité des liens
void RoutingProtocol::ReadLinkQualityFile(const std::string& filename)
{
  std::ifstream file(filename);
  std::string line;
  while (std::getline(file, line)) {
    ...
    m_linkQuality.push_back(row);
  }
}

// Implémentation de la méthode pour obtenir la qualité du lien entre deux nœuds
double RoutingProtocol::GetLinkQuality(int fromNode, int toNode)
{
  return m_linkQuality[fromNode][toNode];
}
...
////////////////////////////////////////////
int RoutingProtocol::GetNodeIndex(Ipv4Address addr)
{
  for (uint32_t i = 0; i < NodeList::GetNNodes(); ++i)
  {
    ...
    {
      if (ipv4->GetAddress(j, 0).GetLocal() == addr)
      {
        return i;
      }
    }
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
  ...
  // Add logic to get link quality and node index
  int fromNode = GetNodeIndex(src);
  int toNode = GetNodeIndex(receiver);
  double linkQuality = GetLinkQuality(fromNode, toNode);

  // Update routing table entries to include link quality in decision
  RoutingTableEntry toOrigin;
  ...
  else
  {
    ...
    toOrigin.SetLinkQuality(linkQuality); // Update link quality
  ...
  if (!m_routingTable.LookupRoute(src, toNeighbor))
  {
    ...
    RoutingTableEntry newEntry(dev,
                               ...
                               linkQuality); // Include link quality
    m_routingTable.AddRoute(newEntry);
  }
  else
  {
    ...
    toNeighbor.SetLinkQuality(linkQuality); // Update link quality
